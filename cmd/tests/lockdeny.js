...
module.exports = {
    ...
    async execute(msg, args) {
        ...
        if (member == msg.author.id) {
            let emb = new EmbedBuilder().setColor('#2b2d31').setDescription(`you can not **deny yourself** from talking .`);
            return msg.channel.send({ embeds: [emb] });
        }
        ...
        if (!new PermissionsBitField(msg.channel.permissionsFor(member)).toArray().includes('SendMessages')) {
            let emb = new EmbedBuilder().setColor('#2b2d31').setDescription(`this user is **already denied** from talking .`)
            return msg.channel.send({ embeds: [emb] });
        }
