...
module.exports = {
    ...
    async execute(msg, args) {
        ...
        if (usr.roles && usr.roles.highest.position > msg.member.roles.highest.position) {
            let emb = new EmbedBuilder().setColor('#2b2d31').setDescription(`you are unable to ban <@${usr.user.id}> as they have a **higher position** than you .`)
            return msg.channel.send({ embeds: [emb] });
        } else if (usr.roles && usr.roles.highest.position > msg.guild.members.me.roles.highest.position) {
            let emb = new EmbedBuilder().setColor('#2b2d31').setDescription(`unable to ban <@${usr.user.id}> as they have a **higher position** than me .`)
            return msg.channel.send({ embeds: [emb] });
        } else {
            try {
                args.shift()
                ...
                let emb = new EmbedBuilder().setColor('#2b2d31').setDescription(`<@${usr.user.id}> was **banned** .`)
                return msg.channel.send({ embeds: [emb] });
            } catch (err) {
                console.error(err);
                let emb = new EmbedBuilder().setColor('#2b2d31').setDescription(`there was a **problem** trying to ban <@${usr.user.id}> .`)
                return msg.channel.send({ embeds: [emb] });
            }
        }
    }
