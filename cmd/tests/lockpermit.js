...
module.exports = {
  ...
  async execute(msg, args) {
    ...
    if (member == msg.author.id) {
      let emb = new EmbedBuilder().setColor('#2b2d31').setDescription(`you are **already permitted** to talk .`);
    ...
    if (new PermissionsBitField(msg.channel.permissionsFor(member)).toArray().includes('SendMessages')) {
      let emb = new EmbedBuilder().setColor('#2b2d31').setDescription(`this user is **already permitted** to talk .`)
      return msg.channel.send({ embeds: [emb] });
    }

    await msg.channel.permissionOverwrites.edit(member, { SendMessages: true });
    let emb = new EmbedBuilder().setColor('#2b2d31').setDescription(`<@${member}> has been **permitted** to talk .`);
    return msg.channel.send({ embeds: [emb] });
  }
}